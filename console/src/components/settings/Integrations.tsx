            ...
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item name={['smtp', 'host']} label={t`SMTP Host`} rules={[{ required: true }]}>
                  <Input placeholder={t`smtp.yourdomain.com`} disabled={!isOwner} />
                </Form.Item>
              </Col>
              <Col span={6}>
                ...
                if (authType === 'oauth2') {
                  return (
                    <>
                      ...
                      >
                        <Input placeholder={t`user@yourdomain.com`} disabled={!isOwner} />
                      </Form.Item>
                          ...
                          if (provider === 'microsoft') {
                            return (
                              <>
                                ...
                                >
                                  <Input.Password
                                    placeholder={t`Client Secret Value`}
                                    disabled={!isOwner}
                                  />
                                </Form.Item>
                              </>
                          ...
                          if (provider === 'google') {
                            return (
                              <>
                                ...
                                >
                                  <Input placeholder={t`Client ID`} disabled={!isOwner} />
                                </Form.Item>
                                <Form.Item
                                  name={['smtp', 'oauth2_client_secret']}
                                  label={t`Client Secret`}
                                  ...
                                  tooltip={t`Find this in Google Cloud Console > APIs & Services > Credentials`}
                                >
                                  <Input.Password placeholder={t`Client Secret`} disabled={!isOwner} />
                                </Form.Item>
                                <Form.Item
                                  ...
                                  rules={[
                                    {
                                      required: true,
                                      message: t`Refresh Token is required for Google`
                                    }
                                  ]}
                                  tooltip={t`Obtain this using the OAuth2 playground or your own OAuth flow`}
                                >
                                  <Input.Password placeholder={t`Refresh Token`} disabled={!isOwner} />
                                </Form.Item>
                              </>
                            )
                          }
    ...
    const handleTestFromDrawer = () => {
      ...
      emailProviderForm
        ...
        .catch((error) => {
          ...
          message.error(t`Please fill in all required fields before testing`)
        })
    }
